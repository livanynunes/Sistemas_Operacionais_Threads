...
public class Car implements Runnable{
	...
	if (cartype==REDCAR) {
            if (inFront.getX() - xpos > 100) {
                xpos += 4;
                if (xpos >= bridgeXLeft & xposOld < bridgeXLeft) controller.entradaCarroVermelho();
		...
		else if (xpos >= bridgeXRight &&  xposOld < bridgeXRight) controller.saidaCarroVermelho();
            }
	} else {
            if (xpos-inFront.getX() > 100) {
                xpos -= 4;
                if (xpos <= bridgeXRight && xposOld > bridgeXRight) controller.entradaCarroAzul();
		...
		else if (xpos <= bridgeXLeft && xposOld > bridgeXLeft) controller.saidaCarroAzul();
            }
	}
    }
